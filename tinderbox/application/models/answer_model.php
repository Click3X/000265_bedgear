...
class Answer_Model extends CI_Model
{
	...
	function manageFile($pFbId){
		...
		$fileanswer = date("U")."_".rand(100, 999).".jpg";
	...
	/*
		Function createthumb($answer,$fileanswer,$new_w,$new_h)
		creates a resized image
		variables:
		$answer		Original fileanswer
		$fileanswer	Fileanswer of the resized image
		$new_w		width of resized image
		$new_h		height of resized image
	*/
	function createthumb($panswer, $pPath, $new_w)
	{
		//echo "answer: ".$panswer."<br/>";
		$system=explode(".",$panswer);
		//echo "system2: ".$system[count($system)-1]."<br/>";
		if (preg_match("/jpg|jpeg|JPG/",$system[count($system)-1])){$im=imagecreatefromjpeg($pPath."/".$panswer);}
		if (preg_match("/png|PNG/",$system[count($system)-1])){$im=imagecreatefrompng($pPath."/".$panswer);}
	...
	/*	$old_x=imageSX($src_img);
		...
		//echo "creating image: ".$system[count($system)-1]." sys1: ".$system[1]." fileanswer: ".$fileanswer;

		if (preg_match("/png/",$system[count($system)-1])){
			imagepng($imageResized,$pPath."/thumb_".$panswer);
		} else {
			imagejpeg($imageResized,$pPath."/thumb_".$panswer);
		}
		imagedestroy($im);
		imagedestroy($imageResized);

		return "thumb_".$panswer;
	}
}
