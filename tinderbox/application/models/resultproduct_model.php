...
class ResultProduct_Model extends CI_Model
{
	...
	function manageFile($pFbId){
		...
		$fileresultproduct = date("U")."_".rand(100, 999).".jpg";
		...
		$config['file_resultproduct'] = $pFbId.".jpg";
	...
	/*
		Function createthumb($resultproduct,$fileresultproduct,$new_w,$new_h)
		creates a resized image
		variables:
		$resultproduct		Original fileresultproduct
		$fileresultproduct	Fileresultproduct of the resized image
		$new_w		width of resized image
		$new_h		height of resized image
	*/
	function createthumb($presultproduct, $pPath, $new_w)
	{
		//echo "resultproduct: ".$presultproduct."<br/>";
		$system=explode(".",$presultproduct);
		//echo "system2: ".$system[count($system)-1]."<br/>";
		if (preg_match("/jpg|jpeg|JPG/",$system[count($system)-1])){$im=imagecreatefromjpeg($pPath."/".$presultproduct);}
		if (preg_match("/png|PNG/",$system[count($system)-1])){$im=imagecreatefrompng($pPath."/".$presultproduct);}
	...
	/*	$old_x=imageSX($src_img);
		...
		//echo "creating image: ".$system[count($system)-1]." sys1: ".$system[1]." fileresultproduct: ".$fileresultproduct;

		if (preg_match("/png/",$system[count($system)-1])){
			imagepng($imageResized,$pPath."/thumb_".$presultproduct);
		} else {
			imagejpeg($imageResized,$pPath."/thumb_".$presultproduct);
		}
		imagedestroy($im);
		imagedestroy($imageResized);

		return "thumb_".$presultproduct;
	}
}
